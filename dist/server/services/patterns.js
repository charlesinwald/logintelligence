...
export function trackErrorPattern(error, aiCategory) {
    ...
    const now = Date.now();
    statements.upsertPattern.run(patternHash, aiCategory || 'Unknown', error.message.substring(0, 200), // Template (truncated)
    now, now, error.severity || 'medium');
    return patternHash;
}
/**
...
export function findSimilarErrors(error, recentErrors, threshold = 0.7) {
    ...
    for (const candidate of recentErrors) {
        if (candidate.id === error.id)
            continue;
        const distance = levenshteinDistance(error.message.toLowerCase(), candidate.message.toLowerCase());
