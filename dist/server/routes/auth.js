...
import { createUser, getUserByEmail, getUserByResetToken, updateUser, emailExists } from '../models/User.js';
 ...
 */
router.post('/api-keys', authenticateUser, (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'Not authenticated' });
        }
 ...
 * Delete an API key
 */
router.delete('/api-keys/:id', authenticateUser, (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'Not authenticated' });
        }
